...
std::string_view inja::ConvertDotToJsonPointer(std::string_view dot, std::string& out) {
  out.clear();
  do {
...
void Renderer::render_to(std::stringstream& os, const Template& tmpl, const json& data) {
  m_data = &data;
  ...
  for (size_t i = 0; i < tmpl.bytecodes.size(); ++i) {
    ...
    switch (bc.op) {
      ...
      case Bytecode::Op::Include:
        Renderer(m_includedTemplates, m_callbacks).render_to(os, m_includedTemplates.find(GetImm(bc)->get_ref<const std::string&>())->second, data);
        break;
