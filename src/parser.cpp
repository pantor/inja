...
Token Lexer::ScanBody(std::string_view close, Token::Kind closeKind) {
again:
...
Parser::Parser(const ParserConfig& parserConfig, const LexerConfig& lexerConfig, TemplateStorage& includedTemplates)
...
bool Parser::ParseStatement(Template& tmpl, std::string_view path) {
  ...
  if (m_tok.text == "if") {
    ...
    if (isNew) included->second = parse_template(pathname);
...
void Parser::AppendCallback(Template& tmpl, std::string_view name,
...
void Parser::ParseInto(Template& tmpl, std::string_view path) {
...
Template Parser::parse(std::string_view input, std::string_view path) {
  Template result;
...
Template Parser::parse_template(std::string_view filename) {
  Template result;
